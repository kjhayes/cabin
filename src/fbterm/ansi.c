        ...
        if(n == 38) {
            ...
            if(s == 5) {
                ...
                if(i + 1 > num_param) {
                    ...
                    LOG(tdata, "SGR 38;5; Too Few Arguments for 8-bit Color Depth!\n");
                }
            } else if(s == 2) {
                // 24-bit color
                if(i + 3 > num_param) {
                    ...
                    LOG(tdata, "SGR 38;2; Too Few Arguments for 24-bit Color Depth!\n");
                }
            } else {
        ...
        if(n == 48) {
            ...
            if(s == 5) {
                ...
                if(i + 1 > num_param) {
                    ...
                    LOG(tdata, "SGR 48;5; Too Few Arguments for 8-bit Color Depth!\n");
                }
            } else if(s == 2) {
                // 24-bit color
                if(i+3 > num_param) {
                    ...
                    LOG(tdata, "SGR 48;2; Too Few Arguments for 24-bit Color Depth!\n");
                }
            } else {
...
handle_csi(struct terminal_data *tdata)
{
    ...
    if(!(0x40 <= c && c <= 0x7E)) {
        // Missing Terminator
        LOG(tdata, "CSI Escape is Missing Terminator (last-char=0x%x)\n", (unsigned int)c);
...
#define MAX_COMMON_PARAMS 32
    ...
    if(strspn(parameter_bytes, ";0123456789") == num_parameter_bytes) {
        ...
        while(iter) {
            if(num_common_params >= MAX_COMMON_PARAMS) {
                LOG(tdata, "Too many common paramters provided to CSI escape sequence!\n");
                break;
            }
            num_common_params++;
            if(strlen(iter) == 0) {
    ...
    if(!is_common) {
        num_common_params = 0;
        LOG(tdata, "Ignoring non-common CSI sequence: terminator=0x%x\n",
                terminator);
        return;
    }
    ...
    switch(terminator) {
        case 'J':
            ...
            switch(common_params[0]) {
                case 0: terminal_clear_cursor_to_end_of_screen(tdata); break;
                case 1: terminal_clear_cursor_to_beginning_of_screen(tdata); break;
                case 2: terminal_clear_entire_screen(tdata); tdata->cursor_x = 0; tdata->cursor_y = 0; break;
                case 3: terminal_clear_entire_screen(tdata); /* Note we should also erase any "scrollback */ break;
                default:
                        LOG(tdata, "CSI n %c, invalid parameter n=%d\n", terminator, common_params[0]);
                        terminal_put_at_cursor(tdata, '?');
                        terminal_advance_cursor(tdata);
                        break;
            }
        ...
        case 'K':
            // Erase in line
            switch(common_params[0]) {
                case 0: terminal_clear_cursor_to_end_of_line(tdata); break;
                case 1: terminal_clear_start_of_line_to_cursor(tdata); break;
                case 2: terminal_clear_cursor_line(tdata); break;
                default:
                        LOG(tdata, "CSI n %c, invalid parameter n=%d\n", terminator, common_params[0]);
                        terminal_put_at_cursor(tdata, '?');
                        terminal_advance_cursor(tdata);
        ...
        case '@':
            // Insert n blanks
            if(common_params[0] < 1) {common_params[0] = 1;}
            for(size_t i = 0; i < common_params[0]; i++) {
        ...
        case 'M':
            ...
            break;
        default:
            LOG(tdata, "Unknown CSI Terminator 0x%x, '%c'!\n", terminator, terminator);
            terminal_put_at_cursor(tdata, '?');
            terminal_advance_cursor(tdata);
            break;
    }
...
__ansi_256_get_color(size_t index)
{
    ...
    if(index < 16) {
        switch(index) {
            ...
            case 4:  c = COLOR_BLUE; break;
            case 5:  c = COLOR_MAGENTA; break;
            case 6:  c = COLOR_CYAN; break;
            case 7:  c = COLOR_WHITE; break;
            case 8:  c = COLOR_BRIGHT_BLACK; break;
            case 9:  c = COLOR_BRIGHT_RED; break;
            case 10: c = COLOR_BRIGHT_GREEN; break;
            case 11: c = COLOR_BRIGHT_YELLOW; break;
            case 12: c = COLOR_BRIGHT_BLUE; break;
            case 13: c = COLOR_BRIGHT_MAGENTA; break;
            case 14: c = COLOR_BRIGHT_CYAN; break;
            case 15: c = COLOR_BRIGHT_WHITE; break;
            default: c = COLOR_BLACK; break; // Should never happen
        }
    } else if (index < 232) {
        // Extended Colors
        c.r = ((index - 16) / 36) * 51;
